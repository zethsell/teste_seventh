      ...
      <div class="w-full flex justify-center p-6">
        <div class="w-full sm:w-1/2 md:1/3 flex flex-col justify-start">
          ...
          <form @submit.prevent="save">
            ...
            <span class="flex justify-start mt-2 font-bold">Nivel</span>
            <select class="w-full p-2 h-12 outline-none border w-full p-2 rounded-sm" v-model="user.level_id">
...
export default {
  ...
  methods: {
    save: async function () {
      ...
      await this.axios.post(process.env.VUE_APP_ROOT_API + 'users', this.user, {headers: authHeader()})
          .then((response) => {
